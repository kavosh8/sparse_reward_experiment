...
def get_hyper_parameters(name):
	meta_params={}
	with open("hyper_parameters/"+name+".hyper") as f:
		lines = [line.rstrip('\n') for line in f]
		for l in lines:
...
def mellowmax(next_q_li,omega):
	'''
	given a batch of s times q values, 
	give mellowmax of each row
	'''
	batch_size=len(next_q_li)
	next_q_max=numpy.max(next_q_li,axis=1).reshape(batch_size,1)
	if omega>29.99999:
		return next_q_max
	next_q_li_offset=next_q_li-next_q_max
	exp=numpy.exp(omega*next_q_li_offset)
	sum_exp=numpy.mean(exp,axis=1,keepdims=True)
	log_sum_exp=numpy.log(sum_exp)/omega

	return log_sum_exp+next_q_max

def save(li_returns,params):
	directory="q_learning_results/"+params['hyper_parameters_name']+'/'
